...
if (setup_eq) {
	...
	if ((eq.class == "Arm") || (eq.class == "Legs") || (eq.class == "Charm")) {
		...
		backing.title = eq.title;
		skill_one = instance_create_depth(x + 3 , y + 40 - 270, -6, obj_equip_skill_popout);	
		skill_one.title = eq.ability1;
		skill_two = instance_create_depth(x + 147, y + 40 - 270, -6, obj_equip_skill_popout);
		skill_two.title = eq.ability2;
		show_debug_message("Set " + eq.title);
	}
	...
	upgrades = instance_create_depth(x + 291, y + 60 - 270, -6, obj_flat_base);
	for (var i = 0; i < 6; i++) {
